...
class RobloxAllySender:
    def __init__(self, roblox_cookie, num_threads):
...
def start_ally_sender(num_threads):
    ally_sender = RobloxAllySender(iterator([RobloxCookie(Bypass(cookie).start_process()) for cookie in COOKIES]), num_threads)
...
# Start threading
threading.Thread(target=start_ally_sender, args=(num_threads,)).start()
